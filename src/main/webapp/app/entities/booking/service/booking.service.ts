...
type RestOf<T extends IBooking | NewBooking> = Omit<T, 'bookingDate' | 'modificationDate'> & {
...
export class BookingService {
  ...
  protected convertDateFromClient<T extends IBooking | NewBooking | PartialUpdateBooking>(booking: T): RestOf<T> {
    return {
      ...
      modificationDate: booking.modificationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBooking: RestBooking): IBooking {
    return {
      ...
      modificationDate: restBooking.modificationDate ? dayjs(restBooking.modificationDate) : undefined,
    };
  }
