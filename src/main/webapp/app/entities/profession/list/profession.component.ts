...
import { EntityArrayResponseType, ProfessionService } from '../service/profession.service';
...
export class ProfessionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IProfession): number => this.professionService.getProfessionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.professions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IProfession[] | null): IProfession[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.professionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
