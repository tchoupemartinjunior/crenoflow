...
type SubjectFormDefaults = Pick<NewSubject, 'id' | 'subjectCycles'>;

type SubjectFormGroupContent = {
  id: FormControl<ISubject['id'] | NewSubject['id']>;
  label: FormControl<ISubject['label']>;
  subjectCycles: FormControl<ISubject['subjectCycles']>;
};
...
export class SubjectFormService {
  createSubjectFormGroup(subject: SubjectFormGroupInput = { id: null }): SubjectFormGroup {
    ...
    return new FormGroup<SubjectFormGroupContent>({
      ...
      label: new FormControl(subjectRawValue.label, {
        validators: [Validators.required, Validators.minLength(1), Validators.maxLength(64)],
      }),
      subjectCycles: new FormControl(subjectRawValue.subjectCycles ?? []),
    });
  }
